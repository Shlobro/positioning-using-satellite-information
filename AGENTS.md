...
- only use dependencies whose licenses are compatible with closed-source commercial use for this repository. before adding or installing a library, verify from primary sources that it does not require this codebase to be open sourced or otherwise impose copyleft source-disclosure obligations. if the license position is unclear, stop and ask the user before proceeding. if during any task you encounter an existing dependency that appears to require open-sourcing this codebase or prohibits commercial use (e.g. GPL, AGPL, SSPL, Commons Clause), immediately notify the user with the library name, its license, and the specific concern before continuing.
- always use CUDA/GPU for any AI or compute-intensive workload. assume the target machine always has a CUDA-capable GPU available. any library or framework that supports CUDA (PyTorch, TensorFlow, OpenCV with CUDA, cuDNN, etc.) must be configured to use it — never fall back to CPU-only mode unless a specific operation has no GPU path.
