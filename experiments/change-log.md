...
- owner: Codex
- files changed: `src/satellite_drone_localization/eval/sequence_policy.py`, `src/satellite_drone_localization/eval/sequence_search.py`, `tests/test_sequence_search.py`, `HUMAN_NEXT_STEPS.md`, developer guides, `experiments/change-log.md`, `final-grand-plan.md`
- intent: Tighten the contained velocity-likelihood scenario so strong RoMa candidates can still be rejected when they disagree materially with the velocity-predicted prior.
- linked run_ids: none yet
- actual result: The sequence-likelihood gate now multiplies motion likelihood, matcher-evidence likelihood, and a new prediction-consistency likelihood based on the distance between the velocity-predicted prior and the accepted RoMa estimate. Deterministic tests now cover helper-level rejection for prediction-inconsistent candidates and a full sequence scenario that falls back with `fallback_roma_sequence_low_likelihood` while retaining the previous accepted state. Agent-run syntax compilation passed for the edited modules; required local batch verification and CUDA replay evidence still need to be run by the user.

- owner: Shlomo / Codex
- files changed: `artifacts/manual-verification/sequence-search-roma-velocity-likelihood/`, `HUMAN_NEXT_STEPS.md`, `experiments/change-log.md`, `experiments/experiment-log.csv`, `final-grand-plan.md`
- intent: Measure whether the new prediction-consistency gate is enough to keep the velocity-likelihood RoMa scenario competitive with the map-constrained temporal baseline.
- linked run_ids: manual `sequence-search-roma-velocity-likelihood-prediction-consistency`
- actual result: Required local verification passed on 2026-04-30 with `scripts/run_pytest_isolation.bat`, ending in `verification_ok`. The Windows CUDA replay then showed that the stricter gate did produce explicit `fallback_roma_sequence_low_likelihood: 2`, but the candidate still degraded badly. `recursive_roma_map_constrained_matcher` remained stronger with `53/92` matches and `4.60m` mean error, while `recursive_roma_velocity_likelihood_matcher` dropped to `23/92` matches and `14.09m` mean error. The comparison deltas were `-9.49m` mean error, `-11.58m` max error, `-25.06m` final error, and `-30` matched frames for the candidate, with recommendation `keep_map_constrained_temporal_gate_as_baseline`. This closes the current Roma velocity-likelihood branch as a negative result.

- owner: Claude
- files changed: `tools/localization_gui/` (new folder: `launch_localization_gui.bat`, `localization_gui.py`, `map_view.py`, `controls_panel.py`, `result_panel.py`, `pipeline_runner.py`, `single_image_input.py`, `__init__.py`, `localization_gui_developer_guide.md`), `tests/test_localization_gui_single_image.py`, `tests/test_localization_gui_pipeline_runner.py`, `root_developer_guide.md`, `tests/tests_developer_guide.md`, `experiments/change-log.md`, `final-grand-plan.md`
- intent: Add an interactive GUI that surfaces the existing matcher and sequence-search pipelines as a real-time demo and research tool, so localization runs can be inspected without writing artifact JSONs.
- linked run_ids: none
- actual result: The new `tools/localization_gui/` tool launches a PyQt6 + pyqtgraph window with a calibrated satellite tile, single-image (`<image>_packet.json` sidecar in `dev-packet-v1` schema) and full replay sequence input modes, click-to-set or lat/lon-text prior, slider-driven search radius, every existing matcher and sequence scenario in the pipeline dropdown, and visual overlays for predicted location, search-radius circle, confidence heatmap (template-match grid for image-baseline pipelines), warped query-image preview, and truth marker when ground truth is available. Headless logic is covered by 5 single-image-input tests and 4 pipeline-runner tests, all of which pass against a synthetic calibrated map fixture. Heatmap surfacing for RoMa certainty is intentionally deferred until it can flow through the matcher diagnostics path. Required local batch verification still needs to be run by the user.

- owner: Codex
- files changed: `tools/localization_gui/localization_gui.py`, `tools/localization_gui/controls_panel.py`, `tools/localization_gui/pipeline_runner.py`, `tools/localization_gui/localization_gui_developer_guide.md`, `tests/test_localization_gui_pipeline_runner.py`, `tests/tests_developer_guide.md`, `root_developer_guide.md`, `experiments/change-log.md`, `final-grand-plan.md`
